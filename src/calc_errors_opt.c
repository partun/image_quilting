 ...
 */
int *
calc_errors_opt(Image *src, Image *out, ImageCoordinates out_coord, int block_size,
    ...
    for (int candidate_y = 0; candidate_y < max_src_y; ++candidate_y) {
        for (int candidate_x = 0; candidate_x < max_src_x; ++candidate_x) {
            ...
            for (int block_y = 0; block_y < y_loop_lim; ++block_y) {
                ...
                for (; block_x < x_loop_lim; block_x+=3) {
   ...
   if (direction == CORNER) {
                ...
                src_idx = (overlap + candidate_y) * src->width + candidate_x;
                out_idx = (overlap + out_coord.y) * out->width + out_coord.x;
                ...
                for (int block_y = overlap; block_y < block_size; ++block_y) {
                    ...
                    for (; block_x < overlap_limit; block_x+=3) {
                        r_error0 = src_rgb0->r - out_rgb0->r;
                        g_error0 = src_rgb0->g - out_rgb0->g;
                        b_error0 = src_rgb0->b - out_rgb0->b;
                        r_error1 = src_rgb1->r - out_rgb1->r;
