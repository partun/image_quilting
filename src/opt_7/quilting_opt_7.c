#include "src/opt_7/calc_errors_opt_7.h"
#include "src/opt_7/min_cut_opt_7.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
    ...
    for (int y = 0; y < out_blocks; y++) {
        for (int x = 0; x < out_blocks; x++) {
            ...
            if (x == 0 && y == 0) {
                ...
                copy_block_opt_7(image, src_coord, out_im, out_coord, block_size);
            } else if (y == 0) {
                // case left overlap
                calc_errors_opt_7(errors, image, out_im, out_coord, block_size,
                                  overlap_size, LEFT);
                ImageCoordinates src_coord = find_best_block_opt_7(errors, image,
                                                                   block_size, candidate_coords);
                CutMatrix *cut = min_cut_opt_7(
                        image,
                        out_im,
                        src_coord,
                        out_coord,
