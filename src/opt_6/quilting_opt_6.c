    ...
    for (int y = 0; y < out_blocks; y++) {
        for (int x = 0; x < out_blocks; x++) {
            ...
            if (x == 0 && y == 0) {
                ...
                copy_block_opt_6(image, src_coord, out_im, out_coord, block_size);
            } else if (y == 0) {
                // case left overlap
                calc_errors_opt_6(errors, image, out_im, out_coord, block_size,
                                  overlap_size, LEFT);
                ImageCoordinates src_coord = find_best_block_opt_6(errors, image,
                                                                   block_size, candidate_coords);
                CutMatrix *cut = min_cut_opt_6(
                        image,
                        out_im,
                        src_coord,
                        out_coord,
