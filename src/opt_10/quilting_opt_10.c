#include "src/opt_10/calc_errors_opt_10.h"
#include "src/opt_10/min_cut_opt_10.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
ImageRGB *quilting_opt_10(
        ImageRGB *image, int block_size, int out_blocks,
        int overlap_size
) {
    ColorOrder color_order = get_image_color_order(image);
    permute_image_colors(image, color_order);
//    print_image_order(color_order);
//    print_image_order(get_image_color_order(image));
    srand(time(NULL));
    ...
    for (int y = 0; y < out_blocks; y++) {
        for (int x = 0; x < out_blocks; x++) {
            ...
            if (x == 0 && y == 0) {
                ...
                copy_block_opt_10(image, src_coord, out_im, out_coord, block_size);
            } else if (y == 0) {
                // case left overlap
                calc_errors_opt_10(errors, image, out_im, out_coord, block_size,
                                   overlap_size, LEFT);
                ImageCoordinates src_coord = find_best_block_opt_10(errors, image,
                                                                    block_size);
                CutMatrix *cut = min_cut_opt_10(
                        image,
                        out_im,
                        src_coord,
                        out_coord,
