 ...
 */
Matrix *calc_overlap_error_opt_6d(
 ...
 */
void mark_cut_path_hori_opt_6d(CutMatrix *cut, int cut_x_idx, int cut_y_idx) {
    char *data = cut->data;
    int width = cut->width;
 ...
 */
void mark_cut_path_vert_opt_6d(CutMatrix *cut, int cut_x_idx, int cut_y_idx) {
    int idx = 0;
 ...
 * 0 and  0 ->  0
 */
CutMatrix *merge_cut_matrix_opt_6d(CutMatrix *cut_0, CutMatrix *cut_1) {
...
CutMatrix *calc_cut_mask_left_opt_6d(Matrix *error_matrix, int block_size) {
...
//    free_matrix(error_matrix);
    ...
    mark_cut_path_hori_opt_6d(cut, cut_x_idx, overlap_height - 1);
    ...
    for (int i = overlap_height - 2; i >= 0; i--) {
        ...
        mark_cut_path_hori_opt_6d(cut, cut_x_idx, i);
    }
...
CutMatrix *calc_cut_mask_above_opt_6d(Matrix *error_matrix, int block_size) {
    int *overlap_errors = error_matrix->data;
    int overlap_width = error_matrix->width;
    int overlap_height = error_matrix->height;
    ...
    // initialize backtracking
    mark_cut_path_vert_opt_6d(cut, overlap_width - 2, cut_y_idx);
    ...
    for (int x = overlap_width - 2; x > -1; x--) {
        ...
        mark_cut_path_vert_opt_6d(cut, x, cut_y_idx);
    }
...
CutMatrix *min_cut_opt_6d(
    ...
    if (direction == ABOVE) {
        Matrix *error_matrix = calc_overlap_error_opt_6d(source_image, output_image, block_coords,
                                                         output_coords,
                                                         block_size,
                                                         overlap);

        CutMatrix *cut = calc_cut_mask_above_opt_6d(error_matrix, block_size);
        return cut;
        ...
        Matrix *error_matrix = calc_overlap_error_opt_6d(source_image, output_image, block_coords,
                                                         ...
                                                         overlap,
                                                         block_size);
        CutMatrix *cut = calc_cut_mask_left_opt_6d(error_matrix, block_size);
        return cut;

    } else if (direction == CORNER) {
        Matrix *error_matrix_left = calc_overlap_error_opt_6d(source_image, output_image,
                                                              block_coords,
                                                              output_coords,
                                                              overlap,
                                                              block_size);
        CutMatrix *cut_left = calc_cut_mask_left_opt_6d(error_matrix_left, block_size);

        Matrix *error_matrix_above = calc_overlap_error_opt_6d(source_image, output_image,
                                                               block_coords,
                                                               output_coords,
                                                               block_size,
                                                               overlap);
        CutMatrix *cut_above = calc_cut_mask_above_opt_6d(error_matrix_above, block_size);

        CutMatrix *cut = merge_cut_matrix_opt_6d(cut_left, cut_above);
