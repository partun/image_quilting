...
int min_err_opt_6c;
double tolerance_6c = 1.1;

void calc_errors_corner_opt_6c(
        int *errors,
    ...
    for (int candidate_y = 0; candidate_y < max_src_y; ++candidate_y) {
        for (int candidate_x = 0; candidate_x < max_src_x; ++candidate_x) {
            ...
            if (error < min_err_opt_6c) {
                min_err_opt_6c = error;
                threshold = (int) (min_err_opt_6c * tolerance_6c);
            }
...
void calc_errors_opt_6c(
    ...
    switch (direction) {
        case CORNER:
            calc_errors_corner_opt_6c(errors, src, out, out_coord, block_size, overlap);
            break;
        case ABOVE:
            calc_errors_above_opt_6c(errors, src, out, out_coord, block_size, overlap);
            break;
        case LEFT:
            calc_errors_left_opt_6c(errors, src, out, out_coord, block_size, overlap);
            break;
        default:
            exit(32);
    }
...
//
//            errors[err_idx] += error;
//            if (errors[err_idx] < min_err_opt_6c) {
//                min_err_opt_6c = errors[err_idx];
//            }
//            err_idx++;
//        }
//    }
 ...
 */
ImageCoordinates find_best_block_opt_6c(
...
*/    int candidate_idx = 0;
    ...
    int threshold = (int) (min_err_opt_6c * tolerance_6c);
